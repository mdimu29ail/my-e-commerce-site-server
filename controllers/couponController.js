...
// // ১. সব কুপন দেখা (Admin or Seller specific)
...
//     res.json(coupons);
//   } catch (error) {
//     res.status(500).json({ message: 'কুপন লোড করা সম্ভব হয়নি' });
//   }
// };

// // ২. নতুন কুপন তৈরি (Admin/Seller)
// exports.createCoupon = async (req, res) => {
//   try {
//     const { code, discount, expiryDate, usageLimit } = req.body;
//     const exists = await Coupon.findOne({ code: code.toUpperCase() });
//     if (exists) return res.status(400).json({ message: 'এই কোডটি আগেই আছে' });
...
//     // সেলার হলে তার আইডি যোগ করা
//     if (req.user.role === 'seller') {
//       couponData.seller = req.user._id;
//     }
...
//       { new: true }
//     );
//     if (!coupon) return res.status(404).json({ message: 'কুপন পাওয়া যায়নি' });
//     res.json(coupon);
//   } catch (error) {
//     res.status(500).json({ message: 'আপডেট ব্যর্থ হয়েছে' });
//   }
// };
...
//     if (!coupon) return res.status(404).json({ message: 'কুপন পাওয়া যায়নি' });
//     res.json({ message: 'সফলভাবে মুছে ফেলা হয়েছে' });
//   } catch (error) {
//     res.status(500).json({ message: 'ডিলিট করা সম্ভব হয়নি' });
//   }
// };

// // ৫. কুপন ভেরিফাই (Public/User)
...
//       return res.status(404).json({ message: 'ভুল বা মেয়াদোত্তীর্ণ কুপন' });
//     }
